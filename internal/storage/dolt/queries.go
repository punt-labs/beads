...
//
// Delegates to issueops.GetReadyWorkInTx for shared query logic, matching
// the pattern used by EmbeddedDoltStore and DoltServerStore. The
// computeBlockedIDsTx adapter preserves DoltStore's per-invocation cache.
func (s *DoltStore) GetReadyWork(ctx context.Context, filter types.WorkFilter) ([]*types.Issue, error) {
	var result []*types.Issue
	err := s.withReadTx(ctx, func(tx *sql.Tx) error {
		var txErr error
		result, txErr = issueops.GetReadyWorkInTx(ctx, tx, filter, s.computeBlockedIDsTx)
		return txErr
	})
	return result, err
}
...
// computeBlockedIDsTx is a transaction-scoped adapter that delegates to
// issueops.ComputeBlockedIDsInTx and populates the DoltStore blocked-ID
// cache. This allows GetReadyWork to delegate to GetReadyWorkInTx while
// preserving the per-invocation caching that the DoltStore provides.
func (s *DoltStore) computeBlockedIDsTx(ctx context.Context, tx *sql.Tx, includeWisps bool) ([]string, error) {
	s.cacheMu.Lock()
	if s.blockedIDsCached && (s.blockedIDsCacheIncludesWisps || !includeWisps) {
		result := s.blockedIDsCache
		s.cacheMu.Unlock()
